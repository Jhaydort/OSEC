...
export function StarRating({ value, onChange, onBlur, error, disabled = false }: StarRatingProps) {
  const [hovered, setHovered] = useState<number | null>(null);
  const highlighted = hovered ?? value;
  return (
    <fieldset disabled={disabled} className="osec-patient-story__rating" aria-describedby={error ? 'story-rating-error' : undefined} aria-invalid={Boolean(error)}>
      ...
      <div className="osec-patient-story__stars" onPointerLeave={() => setHovered(null)}
        ...
        {[1, 2, 3, 4, 5].map((star) => (
          <label className="osec-patient-story__star" key={star}
            onPointerEnter={(event) => { if (!disabled && (event.pointerType === 'mouse' || event.pointerType === 'pen')) setHovered(star); }}>
